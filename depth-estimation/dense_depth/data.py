...
class DataLoader():
    def __init__(self, csv_file='data/nyu2_train.csv',path_to_data_folder='', DEBUG=False):
        self.shape_rgb = (480, 640, 3)
        self.shape_depth = (240, 320, 1)
        self.read_nyu_data(csv_file, path_to_data_folder, DEBUG=DEBUG)

    def nyu_resize(self, img, resolution=480, padding=6):
        from skimage.transform import resize
        return resize(img, (resolution, int(resolution*4/3)), preserve_range=True, mode='reflect', anti_aliasing=True )

    def read_nyu_data(self, csv_file, path_to_data_folder='', DEBUG=False):
        csv_filepath = os.path.join(path_to_data_folder, csv_file)
        csv = open(csv_filepath, 'r').read()
        ...
        self.filenames = [os.path.join(path_to_data_folder, i[0]) for i in nyu2_train]

        # A vector of depth filenames.
        self.labels = [os.path.join(path_to_data_folder, i[1]) for i in nyu2_train]
